...
def get_sentiment_scores(df):
    ...
    tokenizer = AlbertTokenizer.from_pretrained("community/kr-cryptodeberta")
    sentiment_model = AutoModelForSequenceClassification.from_pretrained("community/kr-cryptodeberta")
    scores = np.zeros(3)
...
def index(request):
    ...
    synopex_story_names = ['synopex_story']
    story_board = Board.objects.filter(name__in=synopex_story_names)
    story_posts = {}
    for board in story_board:
        posts = Question.objects.filter(board=board).order_by('-create_date')[:15]
        story_posts[board] = posts
    ...
    ).order_by("-num_answer", "-num_voter", "-create_date")[:8]
